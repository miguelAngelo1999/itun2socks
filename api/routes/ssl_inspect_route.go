...
func sslInspectRouter() chi.Router {
	...
	// CRL and OCSP endpoints — served locally so schannel can verify revocation
	// on leaf certs generated by the MITM interceptor (which include these URLs
	// as CRL Distribution Points / OCSP Server fields).
...
func sslInspectStatus(w http.ResponseWriter, r *http.Request) {
	...
	if fresh {
		// Always run a fresh probe — either via the given proxy or direct
		var status ssl.BumpStatus
		if proxyAddr != "" {
		...
		resp := map[string]interface{}{
			...
			"error":     status.Error,
			// hasCert reflects THIS probe only — don't pull from stale cache
			"hasCert": status.Detected && len(status.InterceptCA) > 0,
		}
		if status.Detected {
	...
	resp := map[string]interface{}{
		...
		"hasCert":   lastSslStatus.Detected && len(lastSslStatus.InterceptCA) > 0,
	}

	if lastSslStatus.Detected {
		if der, _ := ssl.Cached(); len(der) > 0 {
			if info := parseSslCertInfo(der); info != nil {
				resp["certInfo"] = info
			}
...
// sslInspectCRLDer serves a valid empty CRL in DER format.
// This URL is embedded as the CRL Distribution Point in MITM leaf certificates
// so Windows schannel can satisfy its revocation check requirement.
func sslInspectCRLDer(w http.ResponseWriter, r *http.Request) {
	ca := mitm.GetGlobalCA()
	if ca == nil {
...
// sslInspectCRLPem serves the same CRL in PEM format (for OpenSSL-based tools).
func sslInspectCRLPem(w http.ResponseWriter, r *http.Request) {
	ca := mitm.GetGlobalCA()
	if ca == nil {
		http.Error(w, "no CA available", http.StatusNotFound)
		return
...
// sslInspectOCSP returns a minimal "good" OCSP response so schannel is satisfied
// when it tries OCSP before falling back to CRL.
// This is not a full RFC 6960 OCSP responder — it returns a trivial response
// that tells schannel the certificate is not revoked.
func sslInspectOCSP(w http.ResponseWriter, r *http.Request) {
	// Return a minimal 200 with ocsp-response content type.
	// Real OCSP response encoding is complex; a 200 with empty body causes
	// schannel to fall back to CRL which we DO serve properly.
