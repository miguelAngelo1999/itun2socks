...
// Cert file: {configDir}/mitm_ca.crt (PEM, plaintext)
// Key file:  {configDir}/mitm_ca_key.enc (encrypted PEM with "enc:" prefix)
...
// loadCA reads and returns the CA from the existing files on disk.
...
// generateCA creates a new 2048-bit RSA Root CA, saves it to disk, and returns it.
...
// Slow path: generate new leaf cert
...
// generateLeafCert creates a new RSA-2048 leaf certificate for domain signed by the CA.
...
// evictExpiredLocked removes expired entries from the leaf cache.
...
// Fingerprint returns the SHA-256 fingerprint of the Root CA cert
...
func (ca *CA) GeneratedAt() time.Time {
return ca.cert.NotBefore
}
