...
// InspectionList is a thread-safe in-memory set of domain patterns that are
// subject to TLS inspection. Patterns can be exact hostnames ("example.com")
// or wildcard patterns ("*.example.com").
type InspectionList struct {
exact     map[string]bool       // exact hostname → enabled
wildcards []InspectionListEntry // entries whose Pattern starts with "*."
mu        sync.RWMutex
}
...
// It first checks for an exact match, then falls back to wildcard suffix matching.
// A wildcard pattern "*.example.com" matches "example.com" and any subdomain.
...
// Add adds pattern to the list as an enabled entry. If the pattern already
// exists (disabled or enabled), it is re-enabled.
...
// Remove deletes pattern from the list. It is a no-op if the pattern does not exist.
...
// Toggle flips the Enabled flag for an existing pattern. It is a no-op if the
