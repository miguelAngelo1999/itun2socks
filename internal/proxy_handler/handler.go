...
// SetMitmInterceptor registers the MITM interceptor with the tunnel layer.
// Called by the manager when SSL inspection is enabled.
func SetMitmInterceptor(m *mitm.MitmInterceptor) {
tunnel.SetMitmInterceptor(m)
}
