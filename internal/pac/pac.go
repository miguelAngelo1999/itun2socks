...
// Fetch downloads a PAC script from url and returns its JS content.
func Fetch(pacURL string) (string, error) {
	...
	resp, err := client.Get(pacURL)
	if err != nil {
		return "", fmt.Errorf("fetch %s: %w", pacURL, err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("fetch %s: HTTP %d", pacURL, resp.StatusCode)
	}
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return "", fmt.Errorf("read %s: %w", pacURL, err)
	}
	return string(body), nil
}

// IsDirectResult reports whether a FindProxyForURL result means "do not proxy".
// Only a result that leads with DIRECT counts. See the doc comment on the
// registry's ShouldDirect for why.
