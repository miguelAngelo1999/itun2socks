...
func detectMyIP() string {
	...
	for _, iface := range ifaces {
		...
		for _, a := range addrs {
			...
			log.Infoln("[pac] myIpAddress() = %s (from interface %s)", ip.String(), name)
			return ip.String()
		}
	}
	...
	if err == nil {
		...
		if udpAddr, ok := conn.LocalAddr().(*net.UDPAddr); ok {
			log.Infoln("[pac] myIpAddress() = %s (from UDP dial fallback)", udpAddr.IP.String())
...
// ── EvalForURL public API ─────────────────────────────────────────────────────

// EvalForURL evaluates FindProxyForURL against the currently active compiled PAC.
// Returns "DIRECT", "PROXY host:port", or "DIRECT" on any error.
// This is the hot path — called per-connection.
func EvalForURL(url, host string) string {
	compiled := getActiveCompiled()
	if compiled == nil {
		return ""
	}
	return compiled.Eval(url, host)
}
