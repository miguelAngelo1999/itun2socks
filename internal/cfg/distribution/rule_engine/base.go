...
// HTTPContext carries the decoded HTTP request data for inspected connections.
// It is non-nil only when the connection has been TLS-intercepted by the MITM proxy.
...
// HTTPRule extends Rule with an HTTP-layer match method.
// Implementations are only evaluated when HTTPContext is non-nil and Inspected is true.
type HTTPRule interface {
Rule
MatchHTTP(ctx *HTTPContext) bool
}
...
// MatchHTTPContext evaluates HTTP-layer rules (URL-PATH, URL-REGEX, HEADER) against
// the provided HTTPContext. It returns the first matching HTTPRule or ErrNotFound.
// Rules that do not implement HTTPRule are skipped.
// If ctx is nil or ctx.Inspected is false, ErrNotFound is returned immediately.
func (e *Engine) MatchHTTPContext(ctx *HTTPContext) (Rule, error) {
if ctx == nil || !ctx.Inspected {
return nil, ErrNotFound
}

for _, parsed := range e.rules {
httpRule, ok := parsed.Rule.(HTTPRule)
if !ok {
continue
}
if httpRule.MatchHTTP(ctx) {
return httpRule, nil
}
}
return nil, ErrNotFound
