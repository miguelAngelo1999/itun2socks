...
// SetMitmInterceptor registers the MITM interceptor that will be consulted
// for every outbound TCP connection on port 443.
...
// getMitmInterceptor returns the current interceptor (may be nil).
...
// Only consider port-443 connections that have a resolvable hostname.
if metadata.DstPort.String() == "443" {
if interceptor := getMitmInterceptor(); interceptor != nil {
// Resolve hostname: prefer metadata.Host, fall back to DNS cache.
host := metadata.Host
if host == "" {
...
// Signal the waiting goroutine that we handled this connection.
defer ct.Wg().Done()
